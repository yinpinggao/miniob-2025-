    ...
    def __compare_files(self, file1, file2):
        with open(file1, 'r') as f1, open(file2, 'r') as f2:
            ...
            for i in range(max_lines):
                ...
                if line1.upper() != line2.upper():  # 使用 strip 去除行首尾空白
                    _logger.info('Files are different at line %d:\n file1: %s\n file2: %s', i + 1, line1, line2)
                    match = False
                    break

            if match:
                _logger.info('Files are identical: file1=%s, file2=%s', file1, file2)
            else:
                _logger.info('Files are different: file1=%s, file2=%s', file1, file2)
