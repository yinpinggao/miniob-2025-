...
精确检索计算所有向量，保证 top-k；ANN 只搜索部分候选，牺牲 recall 换取速度。IVF 先把向量聚类到多个 list，查询只探测距离最近的若干 list。第 18 题正文只要求无索引精确检索，仓库里的 IVF 是扩展实现，不能混为赛题必做要求。
...
正确说法：`VECTOR(n)`、转换和距离主路径可用，但无括号 `VECTOR` 被实现成 1 维而不是默认 2048 维，最大维度约 16000 而不是 16383，官方同义词也未实现。

不要说：

> DATE_FORMAT 只存在少量显示差异，不影响稳定性。

正确说法：早期年份的 `%Y` 不补足四位，而 `%y` 对短年份字符串直接 `substr(2,2)`，例如年份 1 会抛未捕获异常并终止 observer。
...
正确说法必须带时间和范围，例如：“2026-08-11 在当前容器完成 Debug 全量构建；关闭 LeakSanitizer 后运行 CTest，结果以 `06_source_and_test_index.md` 的本次记录为准；SQL harness 和针对性 SQL 另行说明。”不要把单元测试、赛题 SQL 测试和人工验证混成一句“全部通过”。
